...
class FilterPermissionAction(ContextMenuItem):
    ...
    когда не хватает определенного разрешения. Его можно указать
...
class LoginAction(FilterPermissionAction):
    ...
    на страницу авторизации, если не хватает прав. Однако иногда
    полезно сделать элемент меню с прямо заданным подобным поведением,
    например для Ajax-форм.
    """
